...
public class Player extends BoardObject {
	...
	private HashMap<String, Integer> guesses = new HashMap<String, Integer>();
	/**
	...
	public ArrayList<int[]> applicable(int x, int y){
		ArrayList<int[]> options = moveOptions(x, y);
		ArrayList<int[]> retVal = new ArrayList<int[]>();
		for (int[] opt : options){
			if (opt[0] == 25 || opt[0] == -1 || opt[1] == 24 || opt[1] == -1){
				continue;
			}
			if (canMove(opt[0],opt[1],x,y)){
				retVal.add(opt);
			}
		}
		return retVal;
	...
	public int[] closestDoor(){
		ArrayList<int[]> doors = this.game.getDoors();
		int[] pos = this.getPosition();
		Queue<int[]> frontier = new LinkedList<int[]>();
		frontier.add(pos);
		Set<int[]> explored = new HashSet<int[]>();
		while (true){
			int[] test = frontier.remove();
			explored.add(test);
			for (int[] opt : this.applicable(test[0],test[1])){
				if (!explored.contains(opt)){
					if (listContains(doors,opt)){
						return opt;
					}
					frontier.add(opt);
				}
	...
	public int[] closestUnusedDoor(){
		...
		while (true){
			...
			for (int[] opt : this.applicable(test[0],test[1])){
				if (!explored.contains(opt)){
					if (listContains(doors,opt)){
						Doorway door = (Doorway) board.occupied(opt[0], opt[1]);
						if (this.getGuesses().get(door.getRoomName()) == null){
							return opt;
						}
					}
					frontier.add(opt);
	...
	public int distanceToDoor(){
		...
		frontier.add(new Node(0,pos));
		Set<int[]> explored = new HashSet<int[]>();
		while (true){
			Node test = frontier.remove();
			explored.add(test.getState());
			for (int[] opt : this.applicable(test.getState()[0],test.getState()[1])){
				Node child = new Node(test.getCost()+1,opt);
				if (!explored.contains(child.getState())){
					if (listContains(doors,child.getState())){
